...
static char const *LOG_TAG = "smpboot";

#define AP_STARTUP_VECTOR SMPBOOT_AP_BOOT_CODE_PHYS_BASE >> 12

_Static_assert(
        (AP_STARTUP_VECTOR << 12) == SMPBOOT_AP_BOOT_CODE_PHYS_BASE,
        "AP code address isn't aligned"
);
_Static_assert(AP_STARTUP_VECTOR <= 255, "AP code address is too high");
...
static void start_ap(uint8_t target_apic_id) {
        // Send INIT IPI
        lapic_send_ipi(
                target_apic_id,
                INIT_IPI_BASE_FLAGS | LAPIC_IPI_FLAG_LEVEL_ASSERT
        );
        lapic_send_ipi(
                target_apic_id,
                INIT_IPI_BASE_FLAGS | LAPIC_IPI_FLAG_LEVEL_DEASSERT
        );
...
WARN_UNUSED_RESULT static void *alloc_ap_boot_stack(void) {
...
static unsigned s_booted_ap_count = 0;
static struct SpinLock s_booted_ap_count_lock;
        ...
        *(uint32_t volatile *)(ap_boot_code_var(&smpboot_ap_pml4)) =
        ...
        for (size_t i = 0; i < ap_count; ++i) {
                ...
                uintptr_t initial_rsp =
                        (uintptr_t)stack_base + AP_BOOT_STACK_SIZE;
                *(uintptr_t volatile *)ap_boot_code_array_item(
                        &smpboot_ap_initial_rsps, sizeof(uintptr_t), i
                ) = initial_rsp;
        }
